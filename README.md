...
NOTE: This project has been abandoned due to performance issues. Namely, as more and more states are created by different stackfunctions, Desmos appears to execute their corresponding list actions before discarding the result, even if they aren't the state currently being executed. I have not found a way to overcome this problem. Currently, the following are implemented: console display, kernel loop, processes, process switching, syscall interface, printing, basic memory handling (with allocation, although no reading, writing, or deallocation), basic disk handling (currently read-only), and some simple test programs.
